	...
	foreach ($books as $book) {
		...
		<div class="card-deck col-lg-4 text-center">
			<div id="card<?= $book->getIsbn(); ?>" class="card mb-3">
				<h3 class="card-header" id="cardHeader<?= strip_tags($book->getIsbn()); ?>">titre</h3>
				<div class="marble card-body" id="cardBody<?= strip_tags($book->getIsbn()); ?>"></div>
			</div>
		</div>
		<?php
		$list[] = strip_tags($book->getIsbn());
	}
	?>
</section>
